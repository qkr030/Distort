document.addEventListener('DOMContentLoaded', function() {
    ...
    const buttonContainer = document.querySelector('.button-container'); // 버튼 컨테이너 선택
    ...
    gImages.forEach(image => {
        image.addEventListener('click', function() {
            ...
            // 배경색과 헤드라인 색 변경
            document.body.style.backgroundColor = '#FFF9C1'; // 원하는 색으로 변경
            headline.style.color = 'orange'; // 헤드라인 색 변경
            headline.textContent = 'Clarify'; // 헤드라인 텍스트 변경

            // 버튼 컨테이너 숨기기
            buttonContainer.style.visibility = 'hidden'; // 버튼 컨테이너 숨기기
            buttonContainer.style.opacity = '0'; // 버튼의 opacity를 0으로 설정하여 사라지게 함
            ...
            setTimeout(() => {
                ...
                // 버튼 컨테이너 다시 보이게 하기
                buttonContainer.style.visibility = 'visible'; // 버튼 컨테이너 다시 보이게 설정
                buttonContainer.style.opacity = '1'; // 버튼의 opacity를 1로 설정하여 보이게 함
            }, 2000);
        });
    });
